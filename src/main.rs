...
async fn main() {
    let buttons = [
        ...
        Button::new(buttons::right_buttons_x(), grid::y_pos() + buttons::button_distance_y(), "PLACE WALL", true),
        Button::new(buttons::right_buttons_x(), grid::y_pos() + buttons::button_distance_y() * 2.0, "START FLAG", true),
        Button::new(buttons::right_buttons_x(), grid::y_pos() + buttons::button_distance_y() * 3.0, "END FLAG", true),
        Button::new(buttons::left_buttons_x(), grid::y_pos() + buttons::button_distance_y(), "SEARCH", false),
    ];
    ...
    loop {
        ...
        input_handler.handle_inputs(&mut visual_handler, &mut mode, &mut start_flag, &mut end_flag);
        let zl = visual_handler.zoom_level;
        print!("{zl}\n");
